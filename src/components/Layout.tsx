...
const Layout: React.FC = () => {
  ...
  // Apply Tool Permissions for client role
  const navigationItems: NavigationItem[] = React.useMemo(() => {
    const role = String(profile?.role || '').toLowerCase()
    if (role === 'admin') {
      // Admin sees everything including admin-only items
      return [
        ...baseNavigation,
    ...
    // While loading or before profile is available, show nothing to avoid flicker
    if (initializing || loading || !profile) {
      return []
    }
    // Clients: filter using tool_permissions
    ...
    const filtered = baseNavigation.filter((item) => {
      // If key exists, respect it; if absent, default allow
      return Object.prototype.hasOwnProperty.call(perms, item.name)
        ? !!perms[item.name]
        : true
    })
    return filtered
  }, [profile, baseNavigation, initializing, loading])
  ...
  return (
    <div className="flex h-screen bg-gradient-to-br from-neutral-light/50 via-white to-neutral-light/30">
      ...
      <div className={`${collapsed ? 'w-20' : 'w-72'} bg-white border-r border-neutral-soft/40 transition-all duration-500 ease-out relative overflow-hidden`}>
        {/* Modern Header Section */}
        <div className={`px-6 py-5 border-b border-neutral-soft/30 ${collapsed ? 'flex flex-col items-center' : 'flex items-center justify-between'} bg-gradient-to-r from-primary-dark/5 via-white to-primary-medium/5`}>
          {!collapsed && (
        ...
        <nav className="flex-1 py-4">
          <div className={`space-y-1 ${collapsed ? 'px-3' : 'px-4'}`}>
            {navigationItems.map((item, index) => {
              ...
              return (
                <div key={item.path} style={{ animationDelay: `${index * 30}ms` }} className="animate-fade-in">
                  <Link
                    to={item.path}
                    className={`group relative flex items-center ${collapsed ? 'justify-center p-3' : 'px-3 py-2.5'} text-sm font-medium rounded-xl transition-all duration-300 ${
                      isActive(item.path)
                        ? collapsed
                          ? 'bg-primary-medium text-white shadow-lg scale-105'
                          : 'bg-primary-medium/10 text-primary-dark border-l-3 border-primary-medium'
                        : 'text-neutral-600 hover:bg-neutral-light/80 hover:text-neutral-dark'
                    }`}
                  >
                    <div className={`relative flex items-center ${collapsed ? '' : 'mr-3'}`}>
                    ...
                    {/* Clean hover effect */}
                    <div className="absolute inset-0 rounded-xl bg-gradient-to-r from-transparent to-transparent group-hover:from-neutral-light/20 group-hover:to-neutral-light/10 transition-all duration-300"></div>
                  </Link>
                </div>
              )
            })}
          ...
          {!collapsed && navigationItems.length > 0 && (
            <div className="mx-4 my-4 border-t border-neutral-soft/50"></div>
          )}
        </nav>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        {/* Enhanced Top Header */}
        <header className="bg-white/95 backdrop-blur-md border-b border-neutral-soft/30 shadow-sm">
          <div className="px-8 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center space-x-2">
              ...
              <div className="relative">
                <button className="relative p-3 text-neutral-medium hover:text-primary-medium hover:bg-primary-light/10 rounded-xl transition-all duration-200 group">
                  <Bell className="w-5 h-5 group-hover:scale-110 transition-transform duration-200" />
                  <span className="absolute -top-1 -right-1 w-4 h-4 bg-gradient-to-r from-accent-danger to-accent-danger/80 rounded-full border-2 border-white shadow-lg flex items-center justify-center">
                    <span className="text-[10px] font-bold text-white">3</span>
                  </span>
                  <div className="absolute -top-0.5 -right-0.5 w-2 h-2 bg-accent-danger rounded-full animate-pulse"></div>
                </button>
              </div>
